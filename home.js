...
function loadUserData() {
    ...
    if (nipeg) {
        ...
        db.collection("pegawai").doc(nipeg).get()
            .then((doc) => {
                if (doc.exists) {
                    const data = doc.data();
...
function loadJabatanKosong() {
    ...
    tbody.innerHTML = '<tr><td colspan="28" style="text-align: center;">Loading...</td></tr>';
    ...
    db.collection("pegawai").get()
        .then((querySnapshot) => {
            ...
            if (querySnapshot.empty) {
                tbody.innerHTML = '<tr><td colspan="28" style="text-align: center;">Tidak ada data jabatan kosong</td></tr>';
                return;
            }
        ...
        .catch((error) => {
            ...
            tbody.innerHTML = '<tr><td colspan="28" style="text-align: center; color: red;">Error loading data</td></tr>';
        });
}
...
function loadDetailJabatan(nipeg) {
    ...
    db.collection("pegawai").doc(nipeg).collection("detail_riwayat").get()
        .then((querySnapshot) => {
            if (querySnapshot.empty) {
...
function displayDetailJabatan() {
    ...
    container.innerHTML = infoHTML + '<p style="text-align:center;">Loading detail...</p>';
    
    db.collection("pegawai").doc(selectedJabatan.nipeg).collection("detail_riwayat").get()
        .then((querySnapshot) => {
            if (querySnapshot.empty) {
                container.innerHTML = infoHTML + '<p style="text-align: center; color: #999;">Tidak ada detail riwayat</p>';
                return;
            }
        ...
        .catch((error) => {
            ...
            container.innerHTML = infoHTML + '<p style="color: red; text-align: center;">Error memuat detail riwayat</p>';
        });
}
...
function loadTabelUsulan() {
    ...
    db.collection("usulan").orderBy("Tanggal", "desc").get()
        .then((querySnapshot) => {
            tbody.innerHTML = '';

            if (querySnapshot.empty) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;">Belum ada usulan</td></tr>';
                return;
            }
...
function updateUsulanStatus(docId, newStatus) {
    ...
    db.collection("usulan").doc(docId).update({
        Status: newStatus,
...
function editUsulan(docId) {
    ...
    db.collection("usulan").doc(docId).get()
        .then((doc) => {
            if (doc.exists) {
                const data = doc.data();
                ...
                const newNIPEG = prompt('Edit NIPEG Pegawai:', data.NIPEG_Pegawai_Diusulkan);
                if (newNIPEG === null) return;
                
                const newNama = prompt('Edit Nama Pegawai:', data.Nama_Pegawai_Diusulkan);
...
function deleteUsulan(docId) {
    ...
    db.collection("usulan").doc(docId).delete()
        .then(() => {
