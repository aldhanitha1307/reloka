...
auth.onAuthStateChanged((user) => {
    ...
    // Small delay untuk ensure everything loaded
    setTimeout(() => {
        const nipeg = localStorage.getItem("NIPEG");

        console.log("NIPEG:", nipeg); // Debug
        console.log("isAdmin:", isAdmin(nipeg)); // Debug

        if (!nipeg || !isAdmin(nipeg)) {
            // User biasa - tampilkan access denied
...
const inputs = {
    ...
    NoSKOrganizationalAssignmen: document.getElementById("NoSKOrganizationalAssignmen"),
    TanggalSKOrganizationalAssi: document.getElementById("TanggalSKOrganizationalAssi"),
...
// EVENT BUTTON
// ===============================
document.getElementById("btnSaveOnly")
    .addEventListener("click", () => saveDetailRiwayat(false));

document.getElementById("btnSaveMore")
